...
class Server {
	...
	start(port) {
		...
		this.functions.forEach(func =>
			func.endpoints.forEach(endpoint =>
				this._attachEndpoint(func, endpoint)
			)
		)
		...
		this.app.listen(port, _ => {
			...
			this.functions.forEach(func => {
				...
				func.endpoints.forEach(endpoint => {
					this.log(
						`  ${endpoint.method} http://localhost:${port}${endpoint.path}`
					)
				})
			})
			if (this.staticFolder) {
				...
				this.log(
					`  GET http://localhost:${port}/static (${this.staticFolder})`
				)
				this.app.use('/static', Express.static(this.staticFolder))
			}
			this.log('----')
	...
	setConfiguration(serverlessConfig, servicePath) {
		this.functions = Object.keys(serverlessConfig.functions)
			.map(name => {
				const functionConfig = serverlessConfig.functions[name]
				const [
					handlerSrcFile,
					handlerFunctionName
				] = functionConfig.handler.split('.')
				return {
					name: name,
			...
			.filter(func => func.endpoints.length > 0)
		if (
			serverlessConfig.custom &&
			serverlessConfig.custom.localDevStaticFolder
		) {
			this.staticFolder = path.join(
				servicePath,
				serverlessConfig.custom.localDevStaticFolder
			)
		}
	...
	_executeLambdaHandler(func, event) {
		return new Promise((resolve, reject) => {
			...
			const handle = require(func.handlerModulePath)[
